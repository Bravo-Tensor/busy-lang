 ...
 */
class CapabilityResolver extends events_1.EventEmitter {
    constructor() {
        super();
     ...
     */
    registerResponsibility(definition) {
     ...
     */
    findCapabilities(searchTerm, filters) {
        const results = [];
        // Search capabilities
        ...
        if (!filters?.type || filters.type === 'responsibility') {
            for (const responsibility of this.responsibilities.values()) {
                ...
                const capabilityView = {
                    ...
                    tags: ['responsibility', ...responsibility.tags || []]
                };
     ...
     */
    getMarketplaceInfo() {
        const capabilitiesByProvider = {};
        const capabilityProviderCount = {};
     ...
     */
    validateCompatibility(required, provided) {
        const issues = [];
     ...
     */
    getProvidersForCapability(capabilityName) {
        return Array.from(this.providers.values()).filter(provider => provider.capabilities.includes(capabilityName));
    }
    /**
     * Resolve a single capability
     */
    async resolveSingleCapability(capabilityName, context) {
     ...
     */
    selectBestProvider(providers, context) {
     ...
     */
    detectProviderConflicts(resolvedCapabilities) {
        const conflicts = [];
     ...
     */
    matchesSearch(capability, searchTerm, filters) {
        // Text search
     ...
     */
    calculateSearchScore(capability, searchTerm) {
        const searchLower = searchTerm.toLowerCase();
        let score = 0;
        // Exact name match
     ...
     */
    validateFieldCompatibility(required, provided) {
        const issues = [];
