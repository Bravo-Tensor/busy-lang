...
The architecture follows this flow: [Tool]s generate [Event]s, [Operation]s declare [Trigger]s, and [Trigger]s map Event outputs to Operation inputs.
...
The trigger declaration supports the following fields:

| Field | Required | Description |
|-------|----------|-------------|
| `event_type` | Yes | Event identifier from a Tool's [Events] section |
| `filters` | No | Field-based conditions for selective matching (supports wildcards, AND logic) |
| `queue_when_paused` | No | Queue events when workspace paused (true) or drop them (false) |

Operations can declare multiple triggers to respond to different event types.
...
For example, a `gmail_new_message` event provides fields like `message_id`, `from`, `subject` which the Operation can reference in its [Input] section.
...
This operation executes a BUSY [Operation]. It requires an operation_definition (the BUSY [Operation] to execute) and optionally provided_inputs (data values or state supplied by the caller to satisfy [Input] requirements).
...
3. Execute Steps - Follow the defined [Steps] or instructions within the [Operation]. Follow each step strictly. If you run into an unexpected issue, stop and return an [Error]. If another [Operation] is referenced in the step, execute that operation in its own scope, providing any [Input]s it needs, then wait for it to return to proceed.
